...
(function () {
    ...
    // settings
    emoji.img_path = 'emojify/images/';
    emoji.sheet_path = 'emojify/images/sheet_emoji_64.png';
    ...
    emoji.replacement = function (idx, actual, wrapper) {
        wrapper = wrapper || '';
        if (emoji.colons_mode)
            return ':' + emoji.data[idx][3][0] + ':';
    ...
    emoji.get_unicode = function (str) {
        emoji.init_colons();
        return str.replace(emoji.rx_colons, function (m) {
            var idx = m.substr(1, m.length - 2);
            var val = emoji.map.colons[idx];
            return val ? emoji.colon_replacement(val, idx, ':') : m;
        });
    };

    emoji.colon_replacement = function (idx, actual, wrapper) {
        wrapper = wrapper || '';
        var text_name = (actual) ? wrapper + actual + wrapper : emoji.data[idx][6] || wrapper + emoji.data[idx][3][0] + wrapper;
        emoji.init_env();
        var unicode = emoji.data[idx][0][0];
    ...
    emoji.emojiarea_replace_colons = function (idx) {
        idx = idx.replace(/\:/g, "");
        emoji.init_colons();
        var val = emoji.map.colons[idx];
        return val ? emoji.emojiarea_replacement(val, idx, ':') : "";
    };

    emoji.emojiarea_replacement = function (idx, actual, wrapper) {
        //var img = emoji.data[idx][7] || emoji.img_path + idx + '.png';
        //var title = emoji.include_title ? ' title="' + (actual || emoji.data[idx][3][0]) + '"' : '';
        // var text = emoji.include_text ? wrapper + (actual || emoji.data[idx][3][0]) + wrapper : '';
        var px = emoji.data[idx][4];
        ...
        if (emoji.use_sheet && px != null && py != null) {
            var mul = 100 / (emoji.sheet_size - 1);
            var style = 'background-position:' + (mul * px) + '% ' + (mul * py) + '%;background-size:' + emoji.sheet_size + '00%';
            return style;
            // return '<span class="emoji-outer emoji-sizer"><span class="emoji-inner" style="' + style + '"' + title + '>' + text + '</span></span>';
        } else {
            return "";
//   <span class="emoji emoji-sizer" style="background-image:url(' + img + ')"' + title + '>' + text + '</span>';
        }
        return "";
    };
